...
fn prompt_for_location(shell: &Shell, input: &mut impl BufRead) -> eyre::Result<PathBuf> {
    ...
    let pathname = input.lines().next().ok_or(format_err!("could not read stdin"))??;
    let mut path = if pathname == String::new() {
        default_path
    } else if pathname.starts_with('~') {
        let stripped_pathname =
            pathname.strip_prefix("~/").ok_or(format_err!("computing other user homedirs unsupported"))?;
        let mut p = dirs::home_dir().ok_or(format_err!("could not compute home dir"))?;
